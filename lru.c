...
int lru_evict(void)
{
	...
	return ((end->pte->frame) >> PAGE_SHIFT);
}

void update_frame(struct frame *f1, struct frame *f2, struct frame *target1, struct frame *target2)
{
...
void lru_ref(pt_entry_t *pte)
{
	...
	if (!front)
	{
		...
		update_frame(front, front, f_ptr, f_ptr);
	}
	else
	{
		...
		{ // Update all frame only if pte is in
			if ((*f_ptr).new_frame)
			{
				update_frame((*f_ptr).last_frame, (*f_ptr).new_frame, (*f_ptr).new_frame, (*f_ptr).last_frame);
			}

			(*f_ptr).last_frame = end;
			(*f_ptr).new_frame = front;
			update_frame(end, front, f_ptr, f_ptr);
			front = f_ptr;
		}
	}
}
